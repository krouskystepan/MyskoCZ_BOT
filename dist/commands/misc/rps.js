    ...
    try {
        ...
        if (!guildConfiguration?.gameChannelIds.length) {
            return await interaction.reply({
                content: 'Tento server nebyl ještě nastaven pro používání příkazu `/rps`. Kontaktujte prosím administrátory serveru.',
                ephemeral: true,
            });
        }
        ...
        const targetUser = interaction.options.getUser('hráč', true);
        ...
        const targetUserInteraction = await reply
            ...
            .catch(async (error) => {
            embed.setDescription(`Hra byla zrušena, protože ${targetUser} neodpověděl/a včas.`);
            await reply.edit({
                content: '',
                embeds: [embed],
                components: [],
        ...
        embed.setDescription(`Nyní je na řadě ${interaction.user}! Vyber jednu z možností.`);
        await reply.edit({
        ...
        const initialUserInteraction = await reply
            ...
            .catch(async (error) => {
            embed.setDescription(`Hra byla zrušena, protože ${interaction.user} neodpověděl/a včas.`);
            await reply.edit({
                content: '',
                embeds: [embed],
                components: [],
        ...
        embed.setDescription(`${targetUser} vybral/a ${targetUserChoice?.name} ${targetUserChoice?.emoji} \n${interaction.user} vybral/a ${initialUserChoice?.name} ${initialUserChoice?.emoji}. \n\n${result}`);
        reply.edit({
            content: '',
            embeds: [embed],
            components: [],
